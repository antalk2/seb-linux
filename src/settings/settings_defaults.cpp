...
void finalizeSettings( SebSettings& settings ) {
  ...
  // Trim keys and quitPasswordHash
  ...
  if ( settings.browser.useStartUrlAsHomeUrl &&
       settings.browser.homeUrl.isEmpty()     )
    {
  ...
  if ( (     settings.browser.mainWindow.allowReloading
         &&  settings.browser.mainWindow.showReloadButton
  ...
  if ( (    settings.browser.additionalWindow.allowReloading
         && settings.browser.additionalWindow.showReloadButton

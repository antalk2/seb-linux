    ...
    , application_(application)
{
    ...
    eac_popup_ = new TaskbarPopup(this);
    auto *outer = new QVBoxLayout(eac_popup_);
    outer->setContentsMargins(0, 0, 0, 0);
    outer->setSpacing(0);
    auto *scrollArea = new QScrollArea(eac_popup_);
    ...
    connect(eac_popup_, &TaskbarPopup::popupHidden, this, [this] {
        button_->setHasPopupOpen(false);
    });

    button_->installEventFilter(this);
    eac_popup_->installEventFilter(this);
    updateState();
}
...
void ExternalApplicationControl::handleClicked()
{
    ...
    eac_popup_->showAbove(button_);
}

void ExternalApplicationControl::hidePopupIfInactive()
{
    if (!button_->underMouse() && !eac_popup_->underMouse()) {
        eac_popup_->hide();
    }
}
...
void ExternalApplicationControl::rebuildPopup()
{
    ...
    for (seb::applications::ApplicationWindow *window : application_.typedWindows()) {
        auto *button = new TaskbarButton(eac_popup_);
